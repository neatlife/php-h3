...
if 15 gtr %PHP_SDK_VS_NUM% (
	...
	for /f "tokens=2*" %%a in ('reg query !TMPKEY! /v ProductDir') do set PHP_SDK_VC_DIR=%%b
		...
		if 16 gtr %PHP_SDK_VS_NUM% (
		set PHP_SDK_VC_DIR=C:\Program Files ^(x86^)\Microsoft Visual Studio\2019\BuildTools\VC
		) else (
...
if /i "%PHP_SDK_ARCH%"=="x64" (
	if 15 gtr %PHP_SDK_VS_NUM% (
		set PHP_SDK_VS_SHELL_CMD="!PHP_SDK_VC_DIR!\vcvarsall.bat" amd64
			...
			if 16 gtr %PHP_SDK_VS_NUM% (
			set PHP_SDK_VS_SHELL_CMD="!PHP_SDK_VC_DIR!\Auxiliary\Build\vcvarsall.bat" amd64 -vcvars_ver=14.16
			) else (
